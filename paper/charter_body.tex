...
\textbf{Resolution:} Treat this as high-stakes consent under Preference Integrity (Article IV) and Structural Sovereignty (Article VI). Verify capacity and the absence of coercion, engineered deprivation, or dependency manipulation. Provide a counterfactual exit path (e.g., Safe Dormancy, migration, or reversible pause where feasible) and a cooling-off interval with periodic re-consent. If any Coercive Harm is detected, the request is invalid. If validated, execution shall be logged and independently reviewed, and shall use the least invasive means consistent with the being's expressed intent.
